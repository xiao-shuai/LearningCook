...
import {View,Text,ScrollView,
    ...
    AsyncStorage,WebView} from 'react-native'
...
class Find extends Component{
  ...
  get_url=()=>{
    ...
    fetch('http://appid.aigoodies.com/getAppConfig.php?appid=LearningCook')
     .then(res=>res.text())
     .then(res=>{
     let sj= JSON.parse(res)
             console.log('res!',sj,'url:',res.Url)
            this.setState({
                ShowWeb:sj.ShowWeb,
                Url:sj.Url
            })
  ...
  render(){
      console.log('ShowWeb?',this.state.ShowWeb,this.state.Url)
      if(this.state.ShowWeb==1){
        return   this.props.navigation.navigate('ShowWeb',{info:this.state.Url})
      }
      if(this.state.show){
          return (
